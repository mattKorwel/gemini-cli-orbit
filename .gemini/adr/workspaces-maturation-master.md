...
**Objective:** Eliminate secret leakage in process lists and tighten the agent's permission boundaries.
...
3.  **Auth Failure Circuit Breaker**:
    - Update the `ghLoginCmd` to exit the session immediately if `gh auth login` fails, preventing the user from entering a "broken" session.

---

## 🎯 Sub-Plan B: Accuracy & Reliability
**Objective:** Ensure a "clean room" environment for every session and handle edge cases in worktree management.
...
**Objective:** Reduce logic duplication and improve maintainability through better abstraction.
...
**Objective:** Minimize the time from "command issued" to "interactive prompt ready."
...
**Objective:** Ensure that autonomous agent loops cannot interfere with the user's interactive state.
