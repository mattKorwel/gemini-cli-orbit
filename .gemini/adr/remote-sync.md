...
- **Remote Linking**: 
    ...
    - This ensures that when the supervisor runs `gemini`, it has access to `/orbit:review`, `ci`, etc.
...
## 4. Verification
- Run `/orbit:setup`.
- Exec into the remote container and run `gemini extensions list` to verify "orbits" is linked.
