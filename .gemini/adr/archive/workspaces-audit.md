...
Perform a deep-dive review of the `gemini-orbits-extension` focusing on
**Accuracy, Isolation, Speed, Security, and Code Quality**.
...
- [ ] Review `orbit-policy.toml` for overly broad permissions (e.g., `git` vs
...
- [ ] Ensure that `node` user isolation inside the container is maintained after
      `chown` fixes.
