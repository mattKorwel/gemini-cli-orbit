...
Evaluate and plan the implementation of pre-baking specific PR code into the
development Docker image versus the current dynamic reference clone approach.
...
- **Cloud Build Trigger**:
  ...
  - If `PR_NUMBER` is set, the Dockerfile will `gh pr checkout` the code into
    `/home/node/dev/main`.
- **Naming Convention**:
...
- **Orchestration Logic**:
  - Update `RemoteProvisioner.ts` to first check if an image with tag
    `pr-${prNumber}` exists in the registry.
  - **If exists**: Use that image and skip the `git checkout` phase.
  - **If not**: Fall back to `development:latest` and perform the standard
...
- **Instant Productivity**: Developers drop directly into a warm environment
  with `node_modules` pre-built for that exact PR.
...
- **The Staleness Problem**: As soon as you push a new commit to the PR, the
  baked image is out of date. You'd still need to run `git pull` inside the
  container, partially defeating the purpose.
- **Build Latency**: Every PR push now waits for a full Docker build before the
  "fast" container is available.
...
The storage costs and staleness issues of pre-baking images outweigh the
marginal startup speed gains for our current scale. We prioritize storage
efficiency and automatic branch updates over the few seconds saved during the
initial container spin-up.
