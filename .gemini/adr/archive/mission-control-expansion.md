...
Port high-value automated workflows from the `mk-offload` branch into the Gemini
Orbits Extension.

## 1. Ported Skills

- **`ci`**: Monitor GitHub Actions and automatically extract failure logs.
- **`async-pr-review`**: Background PR review triggers that run on the remote
  worker without blocking the local CLI.
- **`fix-pr`**: Automatic "Self-Healing" loop that waits for CI to fail and then
  attempts a fix.
- **`review-pr`**: Advanced review logic that analyzes failures across multiple
  packages.
