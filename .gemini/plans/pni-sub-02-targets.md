...
2. **Local Target**: Implement `LocalNoopTarget.ts` which just returns a static `ready` state.
3. **GCP COS Target**: Implement `GcpCosTarget.ts` using Pulumi Automation API to manage a GCE instance with COS image.
