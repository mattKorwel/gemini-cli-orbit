...
1. **Interface Cleanup**: Remove `provision` and `setup` from
   `src/providers/BaseProvider.ts`.
2. **Provider Purge**: Delete the implementation of these methods in
   `GceCosProvider.ts` and `LocalWorktreeProvider.ts`.
3. **Internal Helper Removal**: Remove
   `GceConnectionManager.setupNetworkInfrastructure` and other legacy network
   helpers.
4. **Test Alignment**: Update all provider tests to focus on execution rather
   than provisioning.
