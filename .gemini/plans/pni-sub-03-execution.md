...
Update the `OrbitProvider` implementations to consume the new `InfrastructureState` instead of managing their own infrastructure logic.
...
## Tasks
1. **Base Provider**: Update `OrbitProvider` interface or constructor to receive `InfrastructureState`.
2. **GCE COS**: Refactor to use `InfrastructureState.privateIp` for SSH/Docker commands.
