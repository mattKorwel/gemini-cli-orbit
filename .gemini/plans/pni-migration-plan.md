...
This plan outlines the steps to decouple infrastructure provisioning from operational management by introducing a TypeScript-native IaC layer using **Pulumi**.
...
- **State Storage**: Configure Orbit to use a local filesystem backend for Pulumi state (`~/.gemini/orbit/state/`).

## Phase 1: New Abstraction Layer
Create the core interfaces for the decoupled architecture:
- **`InfrastructureProvisioner`**: Defines the `up()`, `down()`, and `refresh()` methods.
...
Implement the first set of remote provisioners in `scripts/infrastructure/targets/`:
- **`GcpCosTarget.ts`**: Translates current `GCE COS` logic.
- **`GcpWorkstationTarget.ts`**: Adds support for managed Google Cloud Workstations.
- **`GcpK8sTarget.ts`**: Adds support for ephemeral pods in GKE clusters.
- **`LocalNoopTarget.ts`**: Maintains interface parity for local worktree development.

All targets use the Pulumi Automation API to manage their stack lifecycle programmatically.
...
- **`WorkstationExecutionProvider`**: Integration with `gh workstation` or custom attach logic.
...
- **Factory**: `InfrastructureFactory.getProvisioner(schematic.provider, schematic.target)`.
- **Flow**:
...
- **E2E Testing**: Verify `orbit liftoff` with a new schematic correctly provisions a VM on GCP.
- **Code Cleanup**: Remove all `spawnSync('gcloud', ...)` provisioning calls and manual "wait for network" logic.
