...
import { SCHEMATICS_DIR } from '../core/Constants.js';
...
function saveRecommendedSchematic(options: {
  ...
  const schematicsDir = SCHEMATICS_DIR;
...
export async function runGcpPrepare(
  ...
  if (options.schematicName) {
    ...
    if (options.apply) {
      saveRecommendedSchematic({
        ...
        projectId,
        zone,
        machineType: options.machineType || 'n2-standard-8',
