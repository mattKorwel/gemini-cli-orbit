...
The **Attach** command allows you to jump directly into an active mission environment. This is the primary way to take manual control of an ongoing mission.

## 🔗 Persistence & Re-attachment

Orbit sessions are persistent (wrapped in `tmux`). You can safely disconnect and re-attach at any time without losing your state or interrupting the agent.
...
- **Context Preservation**: Your shell history, environment variables, and active processes are preserved.
...
- `orbit attach <ID> [action]`: Attach to a specific PR workstream (e.g., `orbit attach 123 review`).
...
- **Hardware Freedom**: Disconnect from your terrestrial machine, and `orbit attach` again later from any device to resume right where you were.

---

*Note: In the Gemini App, you can also use the `/orbit:attach` slash command.*
