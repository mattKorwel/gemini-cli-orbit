...
2. **Dedicated Pipeline**:
   - Use `.gcp/development-worker.yml` for isolated builds.
   - **Tagging Strategy**: 
...
2. **Branch**: `^main$`.
3. **Configuration**: Point to `.gcp/development-worker.yml`.
...
4. **Configuration**: Point to `.gcp/development-worker.yml`.
