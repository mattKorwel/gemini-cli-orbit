...
import { runSplashdown } from './splashdown.ts';
...
describe('runSplashdown', () => {
  ...
  it('should perform repository splashdown when called without --all', async () => {
    ...
    expect(mockProvider.exec).not.toHaveBeenCalledWith(expect.stringContaining('station-supervisor'));
  });
  ...
  it('should perform global mission splashdown when --all is present', async () => {
    ...
    expect(res).toBe(0);
    expect(mockProvider.exec).toHaveBeenCalledWith(expect.stringContaining('station-supervisor'));
    expect(mockProvider.exec).toHaveBeenCalledWith(expect.stringContaining('rm -rf /mnt/disks/data/worktrees/*'));
  });
});
