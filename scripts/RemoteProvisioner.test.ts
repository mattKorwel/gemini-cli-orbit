...
describe('RemoteProvisioner', () => {
  ...
  it('should provision a new worktree and mission capsule successfully', async () => {
    ...
    mockProvider.getCapsuleStatus.mockResolvedValueOnce({ running: false, exists: false });
    // 2. runCapsule -> returns 0
    // 3. getExecOutput (the echo 1 stabilizer) -> returns status 0
    mockProvider.getExecOutput.mockResolvedValueOnce({ status: 0, stdout: '1' }); 
    ...
    expect(mockProvider.runCapsule).toHaveBeenCalledWith(expect.objectContaining({
        cpuLimit: '4',
        memoryLimit: '16g'
    }));
  });
