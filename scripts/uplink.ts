...
export async function runUplink(args: string[]) {
  ...
  console.log(
    `📡 Establishing uplink to remote mission PR #${prNumber} (${action})...`,
  );

  // Check for active tmux sessions
  const tmuxRes = await provider.getExecOutput(
    `tmux list-sessions -F "#S" | grep "mission-${prNumber}-${action}"`,
  ...
  if (tmuxRes.status === 0 && tmuxRes.stdout.trim()) {
    ...
    console.log(
      '❌ No active mission sessions detected in the remote capsule.',
    );
  }
  ...
  if (logRes.status === 0 && logRes.stdout.trim()) {
    ...
    console.log(
      `Tip: To stream live output, run: orbit attach ${prNumber} ${action}`,
    );
  } else {
