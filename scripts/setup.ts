...
async function prompt(
  ...
  const flagOverride = args
    .find((a) => a.startsWith(`--${flagName}=`))
    ?.split('=')[1];

  if (flagOverride !== undefined) {
    logger.debug(
      'PROMPT',
      `Using flag override for ${question}: ${sensitive ? '****' : flagOverride}`,
    );
    return flagOverride;
  }
  ...
  const displayDefault =
    sensitive && defaultValue
      ? `${defaultValue.substring(0, 4)}...${defaultValue.substring(defaultValue.length - 4)}`
      : defaultValue;
...
async function confirm(
  ...
  const args = process.argv.slice(2);
  const autoAccept =
    args.includes('--yes') || args.includes('-y') || !interactive;
  ...
  return new Promise((resolve) => {
    ...
    rl.question(promptMsg, (answer) => {
      rl.close();
      resolve(
        !answer.trim() ? defaultValue : answer.trim().toLowerCase() === 'y',
      );
    });
  });
}
...
async function createFork(upstream: string): Promise<string> {
  ...
  const forkRes = spawnSync('gh', ['repo', 'fork', upstream, '--clone=false'], {
    stdio: 'inherit',
  });
  logger.logOutput(forkRes.stdout, forkRes.stderr);
  if (forkRes.status === 0) {
    const userRes = spawnSync('gh', ['api', 'user', '-q', '.login'], {
      stdio: 'pipe',
    });
...
async function runDesignMode(name?: string): Promise<OrbitConfig> {
  ...
  const projectId = await prompt(
    ...
    existing.projectId || '',
    true,
    'The GCP Project for this infrastructure.',
  );
  const zone = await prompt('GCP Zone', existing.zone || 'us-west1-a', true);
  const machineType = await prompt(
    'GCE Machine Type',
    existing.machineType || 'n2-standard-8',
    true,
    'e.g. n2-standard-8, n2-highmem-16',
  );
  const backendType = await prompt(
    'Connectivity Backend',
    existing.backendType || 'direct-internal',
    true,
    'direct-internal, external, or iap',
  );
  const vpcName = await prompt('VPC Name', existing.vpcName || 'default', true);
  const subnetName = await prompt(
    'Subnet Name',
    existing.subnetName || 'default',
    true,
  );
  const dnsSuffix = await prompt(
    'Regional DNS Suffix',
    existing.dnsSuffix || '',
    true,
  );
  const userSuffix = await prompt(
    'OS Login User Suffix',
    existing.userSuffix || '',
    true,
  );
  const sshSourceRangesInput = await prompt(
    'SSH Source Ranges (comma-separated)',
    existing.sshSourceRanges?.join(',') || '0.0.0.0/0',
    true,
    'Source IP ranges allowed to connect via SSH (e.g., 1.2.3.4/32,5.6.7.8/24)',
  );
  const sshSourceRanges = sshSourceRangesInput
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);

  const cpuLimit = await prompt(
    'Capsule CPU Limit',
    existing.cpuLimit || '2',
    true,
    'Max CPU cores per isolated mission capsule.',
  );
  const memoryLimit = await prompt(
    'Capsule Memory Limit',
    existing.memoryLimit || '8g',
    true,
    'Max RAM per isolated mission capsule (e.g. 8g, 16g).',
  );
  const reaperIdleLimit = parseInt(
    await prompt(
      'Auto-Shutdown Idle Limit (hours)',
      existing.reaperIdleLimit?.toString() || '8',
      true,
      'Number of hours of inactivity before the station auto-shuts down. 0 to disable.',
    ),
  );
...
export async function runSetup(env: NodeJS.ProcessEnv = process.env) {
  ...
  const profileUrl = args
    ...
    ?.split('=')[1];
  let selectedDesign =
    profileUrl || config.profile || globalSettings.activeProfile;

  // Happy Path: No orbit designs found -> Cohesive Setup
  if (!fs.existsSync(PROFILES_DIR))
    fs.mkdirSync(PROFILES_DIR, { recursive: true });
  const localDesigns = fs
  ...
  if (localDesigns.length === 0) {
    logger.info(
      'SETUP',
      'No orbit designs found. Starting cohesive infrastructure setup...',
    );
  ...
  if (shouldRunPrompts) {
    ...
    if (localDesigns.length > 1 && !profileUrl) {
      ...
      localDesigns.forEach((p, i) =>
        logger.info(
          'SETUP',
          `   ${i + 1}. ${p.replace('.json', '')}${p.replace('.json', '') === selectedDesign ? ' (Active)' : ''}`,
        ),
      );
      const choice = await prompt('Select design number', '1');
      selectedDesign =
        localDesigns[parseInt(choice) - 1]?.replace('.json', '') ||
        selectedDesign;
      const designData = JSON.parse(
        fs.readFileSync(
          path.join(PROFILES_DIR, `${selectedDesign}.json`),
          'utf8',
        ),
      );
    ...
    const rawInstanceName = await prompt(
      'GCE Station Name',
      config.instanceName || `gcli-station-${env.USER || 'gcli-user'}`,
      true,
    );
    config.instanceName = sanitizeName(rawInstanceName);
    config.machineType = await prompt(
      'GCE Machine Type',
      config.machineType || 'n2-standard-8',
      true,
    );
    config.terminalTarget = (await prompt(
      'Terminal Target',
      config.terminalTarget || 'tab',
      true,
    )) as any;
    config.imageUri = await prompt(
      'Orbit Docker Image',
      config.imageUri || DEFAULT_IMAGE_URI,
      true,
    );
    config.autoSetupNet = await confirm(
      'Auto-configure Networking (VPC/NAT)?',
      config.autoSetupNet ?? false,
      true,
    );
  } else {
    ...
    config.autoSetupNet =
      args.includes('--auto-setup-network') || config.autoSetupNet || false;

    const mode = hasFlags ? 'Surgical Update' : 'Fast-Path';
    logger.info(
      'SETUP',
      `✨ ${mode} using design '${selectedDesign || 'default'}'`,
    );
  }
  ...
  // Repo Discovery
  const repoInfoRes = spawnSync(
    'gh',
    ['repo', 'view', '--json', 'name,nameWithOwner,parent,isFork'],
    { stdio: 'pipe' },
  );
  let upstreamRepo =
    config.upstreamRepo || `${UPSTREAM_ORG}/${DEFAULT_REPO_NAME}`;
  if (repoInfoRes.status === 0) {
    const _repoInfo = JSON.parse(repoInfoRes.stdout.toString());
    upstreamRepo =
      _repoInfo.isFork && _repoInfo.parent
        ? _repoInfo.parent.nameWithOwner
        : _repoInfo.nameWithOwner;
  ...
  if (shouldRunPrompts) {
    ...
    if (config.userFork?.startsWith(`${currentUser}/`)) {
      ...
      config.userFork = (await confirm(
        `Create personal fork for missions?`,
        true,
        shouldRunPrompts,
      ))
        ? await createFork(upstreamRepo)
        : upstreamRepo;
    }
  }
  ...
  // PAT Management
  if (!fs.existsSync(GLOBAL_TOKENS_DIR))
    fs.mkdirSync(GLOBAL_TOKENS_DIR, { recursive: true });
  const tokenPath = path.join(GLOBAL_TOKENS_DIR, `${repoName}.token`);
  let githubToken = fs.existsSync(tokenPath)
  ...
  if (!githubToken || reconfigure) {
    ...
    let source = githubToken
      ? 'existing'
      : env.WORKSPACE_GH_TOKEN
        ? 'environment'
        : '';

    if (!suggestedToken) {
      const others = fs
        .readdirSync(GLOBAL_TOKENS_DIR)
        .filter((f) => f.endsWith('.token'));
      if (others[0]) {
        suggestedToken = fs
          .readFileSync(path.join(GLOBAL_TOKENS_DIR, others[0]), 'utf8')
          .trim();
    ...
    if (
      suggestedToken &&
      (await confirm(
        `Use ${source} GitHub token for ${repoName}?`,
        true,
        shouldRunPrompts,
      ))
    ) {
      ...
      githubToken = await prompt(
        'GitHub Token',
        '',
        true,
        'Provide a scoped PAT for the remote management container.',
        true,
      );
    }
  ...
  const tempDir = await prompt(
    'Temporary Directory',
    config.tempDir || DEFAULT_TEMP_DIR,
    shouldRunPrompts,
    'Where should mission-specific temporary scripts and logs be stored?',
  );
  const autoClean = await confirm(
    'Auto-cleanup session data?',
    config.autoClean !== undefined ? config.autoClean : true,
    shouldRunPrompts,
  );
  ...
  updatedGlobal.repos[repoName] = {
    ...
    terminalTarget: config.terminalTarget,
    userFork: config.userFork,
  ...
  if (shouldRunPrompts || shellFlag) {
    const wantShell =
      shellFlag ||
      (await confirm(
        'Install "orbit" CLI and autocompletion in your shell profile?',
        true,
        shouldRunPrompts,
      ));
    if (wantShell) {
      const installRes = spawnSync(
        'npx',
        ['tsx', path.join(SCRIPTS_PATH, 'install-shell.ts')],
        { stdio: 'inherit' },
      );
      if (installRes.status !== 0)
        logger.warn(
          'SETUP',
          '⚠️ Shell integration failed, but continuing liftoff...',
        );
    }
  }
  ...
  if (config.autoSetupNet) {
    ...
    await (provider as any).provision({
      setupNetwork: true,
  ...
  if (
    ...
    const shouldProvision = shouldRunPrompts
      ? await confirm(
          `Station not found. Provision it now?`,
          true,
          shouldRunPrompts,
        )
      : true;
    if (shouldProvision) {
      ...
      logger.info(
        'SETUP',
        `Provisioning new station ${config.instanceName}...`,
      );
      const res = await provider.provision({
        setupNetwork: config.autoSetupNet || false,
      });
      if (res !== 0) return res;
      status = await provider.getStatus();
      logger.info(
        'SETUP',
        `✅ Provisioning complete in ${((Date.now() - start) / 1000).toFixed(1)}s.`,
      );
    } else return 1;
  }
  ...
  if (status.status !== 'RUNNING') {
    ...
    logger.info(
      'SETUP',
      `✅ Station awake in ${((Date.now() - start) / 1000).toFixed(1)}s.`,
    );
  }
  ...
  let res = await provider.exec(
    `sudo mkdir -p ${MAIN_REPO_PATH} ${SATELLITE_WORKTREES_PATH} ${POLICIES_PATH} ${SCRIPTS_PATH} ${CONFIG_DIR} ${EXTENSION_REMOTE_PATH}`,
  );
  if (res !== 0) return res;
  res = await provider.exec(
    `sudo chown -R 1000:1000 ${ORBIT_ROOT} && sudo chmod -R 770 ${ORBIT_ROOT}`,
  );
  if (res !== 0) return res;

  logger.info('REMOTE', 'Syncing extension source...');
  res = await provider.sync(EXTENSION_ROOT + '/', `${EXTENSION_REMOTE_PATH}/`, {
    delete: true,
    exclude: ['node_modules', '.git', 'bundle', 'dist'],
  });
  if (res !== 0) return res;
  ...
  res = await provider.exec(
    `sudo docker exec -u node -e GEMINI_API_KEY=dummy ${provider.stationName} /usr/local/share/npm-global/bin/gemini extensions link ${EXTENSION_REMOTE_PATH}`,
  );
  if (res !== 0) return res;

  const netrc = `machine github.com login oauth-basic password ${githubToken}`;
  await provider.exec(
    `echo "${netrc}" | sudo tee /home/${fullUser}/.netrc > /dev/null && sudo chown ${fullUser}:${fullUser} /home/${fullUser}/.netrc && sudo chmod 600 /home/${fullUser}/.netrc`,
  );
