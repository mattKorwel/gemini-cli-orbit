...
export async function runReviewPlaybook(prNumber: string, targetDir: string, policyPath: string, geminiBin: string, logDir: string, missionHeader: string, guidelinesPath?: string) {
  ...
  runner.register([
    ...
    { id: 'static', name: 'Static Standards', cmd: `${geminiBin} --policy ${policyPath} -p "Analyze the diff in ${path.join(logDir, 'diff.log')} against ${rulesReference} and the mission context in ${path.join(logDir, 'context.log')}. Provide a detailed review of code quality, TS types, and architecture."`, timeout: 600000 },
    { id: 'feedback', name: 'Feedback Analysis', cmd: `node scripts/utils/fetch-pr-info.js ${prNumber} | ${geminiBin} --policy ${policyPath} -p "Summarize the unresolved PR feedback provided on stdin. If no feedback is provided, simply reply with 'No unresolved feedback' and exit immediately."`, timeout: 600000 },
    { id: 'proof', name: 'Behavioral Proof', dep: 'build', cmd: `${geminiBin} --policy ${policyPath} -p "Using the build logs in ${path.join(logDir, 'build.log')} and the diff in ${path.join(logDir, 'diff.log')}, physically exercise the new code in the terminal. Provide logs proving it works."`, timeout: 900000 }
  ]);
