...
Save the plan to implementation-plan.md."`;
...
Otherwise, provide specific feedback and reply with 'NO-GO' at the end."`;
    ...
    if (reviewContent.includes('GO') && !reviewContent.includes('NO-GO')) {
      ...
      const revisionCmd = `${geminiBin} --policy ${policyPath} -p "Revise the implementation plan in implementation-plan.md based on this feedback:
${reviewContent}

Save the updated plan to implementation-plan.md."`;
  ...
  const executionPrompt = `Implement Issue #${issueNumber} according to the plan in implementation-plan.md. 
FOLLOW THESE RULES:
...
5. Record your progress in execution-log.md.`;
