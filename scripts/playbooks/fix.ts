...
export async function runFixPlaybook(prNumber: string, targetDir: string, policyPath: string, geminiBin: string, logDir: string, missionHeader: string) {
  ...
  runner.register([
    ...
    { id: 'repair-ci', name: 'CI & Build Repair', cmd: `${geminiBin} --policy ${policyPath} -p "Analyze the CI failures in ${path.join(logDir, 'ci.log')} and ${path.join(logDir, 'build.log')}. Fix the failing tests or lint errors found in the current codebase."` },
    { id: 'address-comments', name: 'Address PR Feedback', cmd: `node scripts/utils/fetch-pr-info.js ${prNumber} | ${geminiBin} --policy ${policyPath} -p "Address the outstanding technical feedback provided on stdin."` }
  ]);
